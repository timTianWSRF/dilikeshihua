...
function tableupdate(){
    ...
    var selectdoc = $("#myid option:selected");
    selectdoc = selectdoc.text();
    console.log(selectdoc);//文件名

    var selectsx = $("#mytype option:selected");
    selectsx = selectsx.text();
    console.log(selectsx);//图层名
    //拼接两个值可确定要操作的表名
      ...
      layer.open({
          ...
          ,yes: function(index, layero){
            ...
            /*  for(var i=0;i<data1.length-1;i++){
             ...
             console.log("我进来了吗");
            //  var all = body.find("#要素编号").value;

             // console.log(all);
              var newdata = [];
              var datadict = {};
              //var iii = $(layero).find('iframe')[0].contentWindow.$("#要素编号").val();
              //console.log(iii);
              var ikun = 0;
              for(var key in data){
                  //var temp = $(layero).find('iframe')[0].contentWindow.$("#" + data1[ikun]['field']).val();
                  var temp = $(layero).find('iframe')[0].contentWindow;

                  console.log(temp);
                  datadict[data1[ikun]['field']] = temp;
                  ikun = ikun + 1;
                  newdata.push(temp);
              }
...
*/
         ...
         /*         obj.update({
                    ...
                    $.ajax({
                                      ...
                                      // dataType:'json',
                                       data:{
                                           ...
                                           sx: selectsx,
                                           currentid:data.要素编号,
                                           currentleixing:data.所属类别,
                                           newdatas:newdata.toString(),
                                           /*id:newid,
                                           leixing:newleixing,
                                           coding:newcoding,
                                           name:newname,
              ...
              console.log("我进来啦");
              //console.log(data1[0]['field']);
              var j = 0;
              for(var key in data){
                  var i = data[key];
                  //console.log(i);
                  body.find("#" + data1[j]['field']).val(i);
                  j += 1;
                 // body.find('#要素编号').val(i);
              }
              //body.find('#id').val(data.id);
              //console.log(data[0]);
              //console.log(body.find("#" + data1[0]['field']).val(data.要素编号));
              //body.find('"#' + data1[0]['field'] + '"');
              //body.find("#id").val(data.要素编号);
              //console.log(data.要素编号);
              //body.find("#id").val(data.id);
